...
test("GW-DECODE-01: codePointAt + 0xFE00 range + Function() sink", () => {
  ...
  const text =
    "var c = s.codePointAt(0); if(c>=0xFE00 && c<=0xFE0F){ " + sink + " }";
...
test("GW-DECODE-01: no match without execution sink", () => {
  ...
  expect(matchSignatures(text).some((m) => m.id === "GW-DECODE-01")).toBe(
    false,
  );
});
...
test("GW-DECODE-02: no trigger on 7 consecutive T1 codepoints", () => {
  ...
  expect(matchSignatures(text).some((m) => m.id === "GW-DECODE-02")).toBe(
    false,
  );
});
...
test("BIDI-FLIP-01: no trigger when bidi override is closed", () => {
  ...
  expect(matchSignatures(text).some((m) => m.id === "BIDI-FLIP-01")).toBe(
    false,
  );
});
  ...
  const good = [
    {
      id: "EXT-01",
      pattern_type: "regex",
      pattern: "suspicious_fn",
      description: "test ext rule",
      confidence: "medium",
      version: 1,
      scope: "text_content",
    },
